...
public class DriveTrainSimulationConfig {
   ...
   *     simulation for the drivetrain. You can specify one factory to apply the same configuration
   *     over all modules or specify four factories in the order (FL, FR, BL, BR).
  ...
  public DriveTrainSimulationConfig(
    ...
    if (swerveModuleSimulationFactory.length == 1)
      this.withSwerveModule(swerveModuleSimulationFactory[0]);
    else if (swerveModuleSimulationFactory.length == 4)
      this.withSwerveModules(swerveModuleSimulationFactory);
    else
      throw new IllegalArgumentException(
          "Module simulation factories length must be 1 or 4, provided "
   ...
   *
   * @param swerveModuleSimulationFactory the new factory (or factories) for swerve module
   *     simulations. You can specify one factory to apply the same configuration over all modules,
