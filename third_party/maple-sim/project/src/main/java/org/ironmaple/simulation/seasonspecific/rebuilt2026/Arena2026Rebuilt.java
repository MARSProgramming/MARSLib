...
public class Arena2026Rebuilt extends SimulatedArena {
  ...
  protected static Translation2d centerPieceBottomRightCorner =
      new Translation2d(7.35737, 1.724406);
  protected static Translation2d redDepotBottomRightCorner = new Translation2d(15.94, 5.53);
  protected static Translation2d blueDepotBottomRightCorner = new Translation2d(0.15, 1.646936);
  ...
  public static final class RebuiltFieldObstaclesMap extends FieldMap {
    ...
    public RebuiltFieldObstaclesMap(boolean AddRampCollider) {
      ...
      addRectangularObstacle(
          ...
          UPRIGHT_Y_LEN,
          new Pose2d(
              UPRIGHT_OFFSET_FROM_END_WALL, UPRIGHT_OFFSET_FROM_SIDE_WALL, new Rotation2d()));
      addRectangularObstacle(
          UPRIGHT_X_LEN,
          UPRIGHT_Y_LEN,
          new Pose2d(
      ...
      if (AddRampCollider) {
        ...
        addRectangularObstacle(
            ...
            new Pose2d(HUB_X, HUB_Y, new Rotation2d()));
   ...
   *
   * <p>This will create an Arena with the ramp areas marked as inaccessible. If you would like to
   * change that use {@link #Arena2026Rebuilt(boolean)}. Additionally, due to performance issues,
   * the arena will not spawn all fuel by default. If you would like to change this, use {@link
   * #setEfficiencyMode(boolean)}
   */
   ...
   * <h2>Creates an Arena for the 2026 FRC game rebuilt </h2>
   *
   * <p>Due to the nature of maple sim the arena cannot be fully simulated, and so either must be
   * non-existent or treated as full colliders. This behavior can be changed with the
   * AddRampCollider variable. Additionally, due to performance issues, the arena will not spawn all
   * fuel by default. If you would like to change this use {@link #setEfficiencyMode(boolean)}
   *
  ...
  public Arena2026Rebuilt(boolean AddRampCollider) {
    ...
    settings.setMinimumAtRestTime(0.05);
   ...
   *
   *
   * <h2>Generates a random number within a range centered on 0 with a variance set by the
   * parameter. </h2>
   *
   ...
   *
   * This method is useful for certain spawners, like the return chutes on the hub, to prevent the
   * game pieces from being returned to the exact same position every time.
   *
   ...
   * @param xVariance The max amount of variance that should be added to the x coordinate of the
   *     game piece.
   * @param yVariance The max amount of variance that should be added to the y coordinate of the
   *     game piece.
   * @param yawVariance The max amount of variance that should be added to the yaw of the game
   *     piece.
   * @param speedVariance The max amount of variance that should be added to the speed of the game
   *     piece.
   * @param pitchVariance The max amount of variance that should be added to the pitch of the game
   *     piece.
   */
   ...
   *
   * This function returns true during autonomous or when shouldClock (set by {@link
   * #setShouldRunClock(boolean)}) is false.
   *
   ...
   *
   * When this is set to false, both goals will always be set to active. This can be useful for
   * testing or to simulate endgame.
   *
   ...
   *
   * This function will dump up to 24 game pieces, dependent on how many game pieces are currently
   * stored in the outpost. For more manual control of the game pieces in the outpost, use {@link
   * #outpostThrow(boolean, Rotation2d, Angle, LinearVelocity)}. To have a human player attempt to
   * throw a game piece into the hub, use {@link #outpostThrowForGoal(boolean)}.
   *
   ...
   *
   * <p>This method comes with variance built in (to simulate human inconsistency) and will
   * therefore only hit about half the time. Additionally, if the hub does not have game pieces
   * stored, this method will not do anything. If you would like to manually control how the human
   * player throws game pieces, use {@link #outpostThrow(boolean, Rotation2d, Angle,
   * LinearVelocity)}
   *
   ...
   *
   * <p>This method comes with variance built in (to simulate human inconsistency). Additionally, if
   * the hub does not have game pieces stored, this method will not do anything. If you would like
   * to have the human player throw at the hub, use {@link #outpostThrowForGoal(boolean)}
   *
   ...
   *
   * <p>Efficiency mode reduces the amount of game pieces on the field to increase performance.
   * MapleSim was not designed with 400 game pieces in mind, and so can struggle with the large
   * number of game pieces present in rebuilt.
   *
