 ...
 *
 * <p>Check<a
 * href='https://shenzhen-robotics-alliance.github.io/maple-sim/simulating-intake/'>Online
 * Documentation</a>
 *
...
public class IntakeSimulation extends BodyFixture {
  ...
  public static IntakeSimulation InTheFrameIntake(
      ...
      IntakeSide side,
      int capacity) {
    return OverTheBumperIntake(
        targetedGamePieceType, driveTrainSimulation, width, Meters.of(0.02), side, capacity);
  }
    ...
    intakeRectangle.translate(
        switch (side) {
          ...
          case RIGHT ->
              new Vector2(
                  0,
                  -driveTrainSimulation.config.bumperWidthY.in(Meters) / 2 - distanceTransformed);
          case FRONT ->
              new Vector2(
                  driveTrainSimulation.config.bumperLengthX.in(Meters) / 2 + distanceTransformed,
                  0);
          case BACK ->
              new Vector2(
                  -driveTrainSimulation.config.bumperLengthX.in(Meters) / 2
                      - distanceTransformed / 2,
                  0);
        });
   ...
   *
   * If the number of pieces added would drive the intake above capacity the intake will only add
   * pieces up to max.
   *
   * @param piecesToAdd The number of pieces to add too the intake.
   * @return Wether or not all game pieces could be added to the intake. Just because this returns
   *     false does not mean that no pieces were added.
   */
  ...
  public final class GamePieceContactListener implements ContactListener<Body> {
    ...
    public void begin(ContactCollisionData<Body> collision, Contact contact) {
      ...
      boolean coralOrAlgaeIntake =
          "Coral".equals(IntakeSimulation.this.targetedGamePieceType)
   ...
   *
   * <p>This method is called from {@link SimulatedArena#simulationPeriodic()} to remove the {@link
   * GamePieceOnFieldSimulation} instances that have been obtained by the intake from the field.
   *
   ...
   *
   * <p>This method allows the user to define a custom condition for determining whether a game
   * piece on the field can be collected by the intake. The condition is specified as a {@link
   * Predicate} that takes a {@link GamePieceOnFieldSimulation} object as input and returns a
   * boolean value.
   *
   * <p>The condition is used to determine whether a game piece should be collected by the intake.
   * If the predicate returns true, the game piece will be collected by the intake. If the predicate
   * returns false, the game piece will not be collected by the intake. Note how this predicate is
   * only called if the game piece is in contact with the intake, and the intake is turned on, and
   * it is the target game piece.
   *
   * @param customIntakeCondition a {@link Predicate} representing the custom condition for intake
   *     eligibility of game pieces on the field
   */
  public void setCustomIntakeCondition(
      Predicate<GamePieceOnFieldSimulation> customIntakeCondition) {
