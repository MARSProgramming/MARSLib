...
public abstract class SimulatedArena {
  ...
  public static BooleanPublisher resetFieldPublisher =
      NetworkTableInstance.getDefault()
  ...
  public static SimulatedArena getInstance() {
    ...
    if (instance == null)
      instance = new org.ironmaple.simulation.seasonspecific.rebuilt2026.Arena2026Rebuilt();
  ...
  private static int SIMULATION_SUB_TICKS_IN_1_PERIOD = 3;
  ...
  public void addToScore(boolean isBlue, int toAdd) {
    ...
    addValueToMatchBreakdown(
        isBlue, DriverStation.isAutonomous() ? "Auto/AutoScore" : "TeleopScore", toAdd);
  }
   ...
   * @param simulationSubTicksPerPeriod the number of Iterations, or {@link #simulationSubTick(int)}
   ...
   *
   * <p>This allows you to register custom actions that will be executed at a high frequency during
   * each simulation sub-tick. This is useful for tasks that need to be updated multiple times per
   * simulation cycle.
   *
   ...
   *
   * @param isBlueTeam Wether to add to the blue teams match breakdown or the red teams match
   *     breakdown
   ...
   *
   *
   * <h2>Defaults a value to 0 and creates it in the match breakdown. This is useful on startup to
   * make sure all match breakdown values display before they are first updated</h2>
   *
   ...
   *
   *
   * <h2>Adds too a value in the scoring breakdown. If value does not already exist in the scoring
   * breakdown it will be defaulted to 0 and then added too
   *
   * @param isBlueTeam Wether to add to the blue teams match breakdown or the red teams match
   *     breakdown
   ...
   *
   *
   * <h2>Shuts down the current SimulatedArena and stops all simulation so that simulatable objects
   * may be added to a new arena </h2>
   */
   ...
   *
   * <p>If not configured through {@link SimulatedArena#overrideSimulationTimings(Time, int)} , the
   * simulator will iterate through 5 Sub-ticks by default.
   *
   ...
   *
   * @return all grounded (aka not projectile) pieces on the field as a set of
   *     GamePieceOnFieldSimulation objects
   */
