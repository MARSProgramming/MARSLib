...
```
  ...
  <div id="sm-sim" >
    <div >Superstructure Collision Sequencer</div>
    
    <div >
       <button id="smStow" >STOW</button>
       <button id="smIntake" >INTAKING</button>
       <button id="smScore" >HUB SCORE</button>
    </div>
    ...
    <div >
      ...
      <div >
        <div>
            <div >SUPERSTRUCTURE TARGET: <span id="smTargetLbl" >STOW</span></div>
            <div >ACTIVE CONSTRAINTS:</div>
            <ul id="smLogList" >
            </ul>
        </div>
        <div  id="smFinalLbl">✔ MECHANISMS ALIGNED</div>
      </div>
      
      {/* Mechanism Canvas */}
      <div >
          <canvas id="smCanvas" width="250" height="200" ></canvas>
      </div>
    </div>
  </div>
