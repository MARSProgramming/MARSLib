...
<main className="container">
  ...
  <p>To ensure code is evaluated safely every 20ms, WPILib enforces the <strong>Command-Based Paradigm</strong>.</p>

  <h2>1. Subsystems (The Nouns)</h2>
  <div className="callout callout-info">
    <h4>Persistent Wrappers</h4>
    <p>A Subsystem represents physical hardware on the robot (e.g., <code>ElevatorSubsystem</code>, <code>ShooterSubsystem</code>). It is instantiated exactly once on boot and persists forever.</p>
  </div>
  <ul>
  ...
  <p>Commands are discrete actions (e.g., <code>FireShooterCommand</code>, <code>ScoreAmpCommand</code>).</p>
  <ul>
  ...
  <p>You cannot blindly command motors in FRC directly from driver inputs. We use the <code>RobotContainer.java</code> class as a strict wiring harness.</p>
