...
**Avoid On-RIO A* Calculation**
    ...
    Calculating an entire field NavGrid requires traversing a massive 2D array graph. Executing this on the core RoboRIO thread can cause devastating loop overruns (exceeding 20ms). Always offload advanced Pathfinding math to the <strong>Driver Station Laptop</strong> or a dedicated coprocessor.
:::
